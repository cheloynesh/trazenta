...
class SixMonthComissionController extends Controller
{
    public function index(){
        ...
        $users = DB::table('users')->select('SixMonth_fund.id as nucid',DB::raw('CONCAT(IFNULL(users.name, "")," ",IFNULL(users.firstname, "")) AS usname'),
            ...
            ->join('SixMonth_fund',"fk_client","=","Client.id")
    ...
    public function ExportPDF($id,$month,$year,$comition,$regime){
        ...
        $value = $this->calculo($id,$comition,$regime);
    ...
    public function GetInfoComition(Request $request)
    {
        ...
        $values = $this->calculo($request->id,$request->comition,$request->regime);
        ...
        'ret_isr'=>$values["ret_isr"], 'ret_iva'=>$values["ret_iva"], 'n_amount'=>$values["n_amount"],'paid'=>$values["pay"]]);
    }

    public function calculo($id, $comition, $regime)
    {
        ...
        $nuc = DB::table('SixMonth_fund')->select("amount","currency","paid")->where('id',$id)->first();
        ...
        'ret_iva'=>$ret_iva, 'n_amount'=>$n_amount, 'pay'=>$nuc->paid);
    ...
    public function SetPayment(Request $request)
    {
        ...
        return response()->json(['status'=>true, 'message'=>"Pago Actualizado"]);
    }
}
